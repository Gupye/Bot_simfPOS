...
def check_rest_code():
    ...
    req = "rest_req.xml"
    resp = "rest.xml"
    log = setings.log_rkeeper
    pasw = setings.pass_rkeeper
    ...
    try:
        ...
        res = False
    text = open("a/rest.xml", 'r', encoding="UTF-8").read()
    soup = BeautifulSoup(text, "lxml")
    rest_code = soup.find('rk7queryresult').find('systeminfo').get('restcode')
    return str(rest_code)
...
def centr2_del(text, four=True):
    if len(text) == 3:
        if text[0] != 'Всего':
            return 'Блюдо            Кол-во  Сумма\n'
        else:
            return f'{text[0]}          {text[1]}   {text[2]}'
    if len(text) > 3:
        a = f'{text[0]} {text[1]} {text[2]}\n' \
            f'{text[3]}{text[4]}\n' \
            f'{text[5]}{text[6]}\n'
        return a
    else:
...
def parse_time_sell():
    ...
    text = open("a/time_sell_report.xml", 'r', encoding="UTF-8").read()
    soup = BeautifulSoup(text, "lxml")
    count = soup.text
    asss = count.split("\n")
    tempmess = []
    ...
    open('temp_reports/time_sell.txt', 'w', encoding='UTF-8').write(endmessage)
...
def bounces_from_receipts():
    ...
    tem = f'xmltest.exe {addr}:{port_xml} {req} {resp} / {log}:{pasw}'
    try:
        a = subprocess.run(tem, check=True, shell=True, cwd='a', stdout=subprocess.PIPE).stdout.decode(
            encoding='UTF-8')

        res = True
    except:
        res = False
    text = open("a/boun_report.xml", 'r', encoding="UTF-8").read()
    soup = BeautifulSoup(text, "lxml")
    count = soup.text
    asss = count.split("\n")
    tempmess = []
    ...
    for i in range(0, len(asss) - 4):
        ...
        if normal_str(asss[i]) == 'ok':
            ...
            endmess += asss[i] + '\n'
    endmessage = delete_space(endmess)
    open('temp_reports/boun_rec.txt', 'w', encoding='UTF-8').write(endmessage)
